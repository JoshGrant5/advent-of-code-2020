...
def count_trees(path, x, y)
  ...
  (1..y).each do |num|
    grid.shift() # we won't count the starting row(s)
  end
...
def multiply_slopes(path, slopes) 
  output = 1
  slopes.each do |pair|
    output *= count_trees(path, pair[0], pair[1])
    puts count_trees(path, pair[0], pair[1])
    puts output
  end
...
puts multiply_slopes(path, slopes)